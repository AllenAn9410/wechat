...
public class CustomAutoGenerator {
     ...
     * 生成代码
     */
    public void execute(Map<String,String> tableRef) {
        logger.debug("==========================准备生成文件...==========================");
        // 初始化配置
        if (null == config) {
        ...
        Map<String,TableInfo> tableInfoMap = configBuilder.getTableInfoList().stream().collect(Collectors.toMap(TableInfo::getName,(tb) -> tb));

        tableInfoMap.values().forEach((TableInfo tab) ->{
            List<TableField> fieldList = new ArrayList<>();
            tab.getFields().forEach((TableField field)->{
                CustomTableField customTableField = new CustomTableField(field);
                String mapKey = tab.getName() + "." + field.getName();
                String refTableName = tableRef.get(mapKey);
                if(StringUtils.isNotBlank(refTableName)){
                    String[] refTable = refTableName.split("\\.");
                    TableInfo tableInfo = tableInfoMap.get(refTable[0]);
                    customTableField.setRefTable(tableInfo);
                    if(refTable.length > 1) {
                        customTableField.setRefField(refTable[1]);
                    }
                }
                fieldList.add(customTableField);
            });
            tab.setFields(fieldList);
        });

        // 模板引擎初始化执行文件输出
        templateEngine.setTableInfoMap(tableInfoMap);
